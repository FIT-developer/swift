...
- **stepper 樣式不同**：本 modal vehicle grid 用 text + `icons/up-and-down`（縱向上下箭頭）；既有 `landing.html` 訂房明細「間數」欄與訂單條件 column 用 `[-] [N] [+]`（兩側 44px 按鈕，定義在 `app.css .stepper`）。屬兩種獨立 stepper 樣式，是否合併或保留差異待確認。
