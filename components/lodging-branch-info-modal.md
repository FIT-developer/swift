...
**Figma Node ID**：見下方「Figma RWD 來源」表（desktop current `2146:107443` / legacy desktop `2129:79343` / 640~767px `2129:80387` / <=639px `2129:80629`）
...
| 斷點 | Node ID | 尺寸 | 說明 |
|---|---|---|---|
| >=768px (desktop current) | `2146:107443` | 820x1554（section）；內層 modal 800x2218 | 2026-08-04 新版，新增 5 張文案卡，飯店設施項目改為右欄 card |
...
| 7 | 飯店名稱（同上，2026-08-04 新增） | 聯絡電話（同上，2026-08-04 新增） |
| 8 | 服務傳真（同上，2026-08-04 新增） | 地址（同上，標題旁 `icons/postoffice`，2026-08-04 新增） |
| 9 | 入住時間（同上，2026-08-04 新增） | 飯店設施項目（chip 多選，見下方「飯店設施項目」段；2026-08-04 改為右欄 card） |
...
- 原本誤判所有欄位是「label 在上、input 在下」堆疊排列；重讀後確認
  **除了 4 張文案卡片（飯店介紹/設施/提醒事項/交通）是 label 在上之外，
  其餘所有欄位（代號/排序/飯店名稱/英文名稱/Email/聯絡電話/傳真/地址外層
...
飯店名稱 / 英文名稱 / Email / 聯絡電話 / 傳真 / 街道等欄位，Figma 顯示的字面內容就是文字 `placeholder`。**實作時直接照字面渲染 `placeholder` 這個字串作為欄位內容**，不當作 HTML `placeholder` 屬性、不當空值處理。理由：這批資料要先給後端看欄位長什麼樣子，後端接手後會自行替換成真實內容。

企業官網欄位例外：Figma 顯示具體示例值 `www.google.com.tw`，視為「已填資料」示例，同樣照字面內容渲染。

---

## 文案卡片（飯店介紹文案 / 飯店設施文案 / 飯店提醒事項文案 / 飯店交通文案 / 飯店名稱 / 聯絡電話 / 服務傳真 / 地址 / 入住時間）

9 張卡片結構一致，完整規格（空狀態/編輯中/已儲存/多語系 tabs/翻譯按鈕/全部儲存-取消暫存規則/SunEditor 工具列/套件研究）已獨立寫在
...
高度行為（2026-08-04 使用者截圖覆核）：同一列左右兩張文案卡必須填滿
grid row 高度。飯店介紹文案因有多語系 tabs，空狀態自然高度會高於右欄
靜態空狀態；右欄的飯店設施文案需垂直拉伸到與左欄同高，下一列的飯店
提醒事項文案 / 飯店交通文案也沿用同一套拉伸規則。靜態空狀態內容
（`尚未填寫` + 編輯按鈕）在拉伸後的 card 內垂直置中。
...
9 張卡片**只有「飯店介紹文案」補上完整 SunEditor UI**（語言 tabs、
SunEditor 佔位工具列、翻譯按鈕、全部儲存/取消，見
`components/lodging-branch-suneditor.md`）；「飯店設施文案」「飯店提醒
事項文案」「飯店交通文案」「飯店名稱」「聯絡電話」「服務傳真」「地址」
「入住時間」維持靜態空狀態顯示（`尚未填寫` + 編輯按鈕，點擊不觸發任何
行為），待之後有需要再個別補上。

2026-08-04 使用者確認：新版 Figma 截圖中的「飯店介紹文案」沒有顯示語言
tabs，但已完成的多語系編輯功能不可回退；本輪只新增新版多出的卡片與調整
飯店設施項目位置。
...
右欄卡片（跟文案卡片同樣 bg/border/radius/padding），內含一排 pill chip：
...
| Breakpoint: <=639px | 欄位改單欄堆疊（不再並排），每個欄位獨佔滿版寬度；文案卡片與飯店設施項目 card 跟隨單欄堆疊 |
