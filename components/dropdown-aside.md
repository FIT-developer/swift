...
│   └── TEXT "主項目 一"  16px  Regular  Color/Neutral/800
...
│   │   └── TEXT "主項目 二"  16px  Regular  Color/Neutral/800
│   └── icons/down (24×24, x:112)      ← 展開狀態箭頭（向下）
└── Frame 5 — 子項目區 (136×64px, y:28)
    ├── Frame 4 — 垂直連接線 (16×109px, x:0)
    │   └── Line 1  stroke Color/Neutral/300（左側縱線）
    └── Frame 2 — 子項目列表 (112×64px, x:24)
        └── Frame 3
            ├── Texts sm "子項目 一" (y:0)   → bg Color/Neutral/100（選中）+ icons/cursor overlay
            ├── Texts sm "子項目 二" (y:24)  → bg Color/Neutral/0（未選中）
            └── Texts sm "子項目 三" (y:48)  → bg Color/Neutral/0（未選中）
```
...
|---|---|---|
| 高度 | `24px` | — |
| 主項目文字 | `Texts / base (16px Regular)`，Color/Neutral/800 | — |
...
| 屬性 | 值 | Token |
|---|---|---|
| 縱向連接線 | stroke `Color/Neutral/300` | `Color/Neutral/300` |
| 子項目列表 x | `24px`（連接線寬 16 + gap 8） | — |
| 子項目高度 | `16px`（sm 字號） | — |
| 子項目間距 | `8px`（y:0, y:24，高 16 → gap 8） | `Spacing/8` |
| 子項目文字 | `Texts / sm (12px Regular)`，`Color/MenuItem/Default` | `Color/MenuItem/Default` |

### 子項目狀態

| 狀態 | 背景 | 說明 |
|---|---|---|
| 選中（active） | 透明（無 fill） | 顯示父層黃底 `Color/SubItem/Selected`（`Color/Tab/Yellow`） |
| 未選中 / hover | `Color/Neutral/0` | `Color/Neutral/0` |
...
2. **左側縱線**：`border-l border-text-placeholder`，高度隨子項目數量延伸
3. **選中子項目**：背景透明（父層 `Color/SubItem/Selected`），其餘 `bg-white hover:bg-surface-hover`
