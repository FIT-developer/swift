...
### 「複製」按鈕目前無點擊行為（2026-07-31 使用者確認，非疏漏）

「複製」按鈕（啟用中卡片）目前是純視覺、點擊無反應。使用者已確認：複製
房型是要交給後端處理的行為（複製一筆完整房型資料、產生新房型編號等），
prototype 前端階段不需要、也不應該自己模擬這個邏輯，等後端介面確定後再接。
不要在沒有後端規格的情況下自行猜測補一個前端假複製行為。
...
| Mobile~tablet | `2140:106747`（2026-07-31 使用者修正版，取代舊
`2137:103071`；舊 frame「content」誤包兩層造成多一塊底色，已在 Figma
修掉，只剩一層 content） | 767x2974 | 卡片單欄滿寬（727px），內容置中同 desktop |
| Mobile | `2140:106898`（2026-07-31 使用者修正版，取代舊 `2137:103890`，
同一次修正） | 375x3182 | 卡片單欄滿寬（335px）；按鈕 3 顆時換行：修改+複製一行，停用置中另一行；
