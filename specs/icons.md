...
- 格式：`kebab-case`，對應 Figma 原名 `icons/xxx`
- 有色彩語意的 icon 加上顏色後綴：`check-green`, `failure-red`, `warning-orange`, `trend-up-red`, `trend-down-green`, `trash-can`（紅 stroke）
- 相同 Figma 原名但設計不同的 icon，以語意區分命名（如 `system` vs `operation-system`）
- 例外：`google`, `logo` 保留品牌原色，不做 `currentColor` 處理
...
| arrow-right | assets/icons/arrow-right.svg | 24×24 | 向右箭頭（大） | icons/arrow-right |
| attached-link | assets/icons/attached-link.svg | 24×24 | 附加連結 | icons/attached-link |
| attached-link-moved | assets/icons/attached-link-moved.svg | 24×24 | 連結已移動 ⚠️ stroke #E1E1E0 | icons/attached-link-moved |
| bed | assets/icons/bed.svg | 24×24 | 床型/房型 | icons/bed |
| bulletin | assets/icons/bulletin.svg | 24×24 | 公告/通知 | icons/bulletin |
| calendar | assets/icons/calendar.svg | 24×24 | 日曆/日期選取 | icons/calendar |
| car | assets/icons/car.svg | 24×24 | 車輛/交通 | icons/car |
| cart | assets/icons/cart.svg | 24×24 | 購物車 | icons/cart |
...
### 導航類
`up`, `down`, `left`, `right`, `arrow-left`, `arrow-right`, `arrow-down-drop`, `arrow-drop-up`, `direction-left`, `return`, `up-and-down`, `flow`
...
### 旅宿業務專屬類
`bed`（床型）, `night`（夜數）, `hotel`（飯店）, `door`（房門）, `reservations`（預約）, `inventory`（庫存）, `duration`（住宿期間）, `count`（數量計算）

### 人員 / 帳號類
`person`, `person-md`, `logout`, `service`（客服）

### 系統 / 工具類
`system`（齒輪設定）, `operation-system`（作業系統操作）, `help`, `language`, `pos`, `home`, `dashboard`, `cursor`

### 交通 / 地點類
`car`, `road`, `transit`, `pin`

### 業務特殊類
`advertisement`, `ai`, `google`（第三方登入）, `logo`（品牌）, `flow`（流程）
...
| 檔名 | 色值 | 原因 |
|---|---|---|
| `check-green.svg` | `#2ACA18` | 成功狀態，語意綠 |
| `failure-red.svg` | `#E12129` | 失敗狀態，語意紅 |
| `warning-orange.svg` | `#E05216` | 警告狀態，語意橙 |
| `trend-down-green.svg` | `#2ACA18` | 價格下降=正向，語意綠 |
| `trend-up-red.svg` | `#E12129` | 價格上升=負向，語意紅 |
